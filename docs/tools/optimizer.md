...
- перейти в оптимизатор можно из уведомления (см.выше) или из меню :fas fa-compress-arrows-alt: "Дополнительные инструменты". В этом случае будет использован текущий активный трек (сегмент). Трек/сегмент должен содержать хотя бы 10 точек, чтобы его можно было открыть в Оптимизаторе.
