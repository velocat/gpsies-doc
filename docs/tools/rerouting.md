...
**🗺️ Краткое описание** Рероутер — это специализированный инструмент в редакторе треков, который используется как одна из возможностей обработки выделенного фрагмента трека. 
...
Включает/выключает отображение тултипов (подсказок) над точками на карте. Состояние синхронизировано кнопке включение/выключение подсказок в блоке-легенде "маршруты/маневры" и в конфигурации "Настройки маршрутизации" (т.е. при изменении в одном из люых мест, остальные перключатся в это состояние автоматически). Эта настройка сохраняет свое состояние в браузере, если разрешено (при повторном открытии редактора в этом браузере будет включено последнее состояние)
----
